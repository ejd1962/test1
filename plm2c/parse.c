...
{
	PARAM_LIST_t	*list_ptr, *param_ptr;
	int		        token_class;
	TOKEN_t		    sep_token;
...
{
	DECL_t		    *extra_decl, *extra_decl_ptr;
	DECL_t		    *list, *list_ptr;
	DECL_MEMBER_t	*extra_list_ptr;
	DECL_MEMBER_t	*el_ptr, *last_el_ptr, *next_el_ptr;
	DECL_MEMBER_t	*extra_el_ptr;
	int		        token_class;
	TOKEN_t		    *decl_token;
	DECL_ID_t		*var_ptr, *last_var_ptr, *next_var_ptr;
	DECL_ID_t		*extra_last_var_ptr;
	PARAM_LIST_t	*param_ptr, *last_param;
...
{
	int		        token_class;
	BOOLEAN_t		left_shift, right_shift;
	char		    *new_func;
...
)
{
	...
	TOKEN_t		    param_token, attrib_token, type_token;
	int		        token_class, next_token_class;
	DECL_t		    *decl_list, *extra_decl_list;
	PARAM_LIST_t	*param_list, *param_ptr;
	DECL_MEMBER_t	*decl_ptr;
	DECL_ID_t		*decl_id;
	BOOLEAN_t		extern_proc, got_type, interrupt_proc;
	char		    *tmp_text_ptr;
 ...
 */
	...
	if (token_class == OPERATOR) {
		if (token.token_type != EQUAL) {
			sprintf(msg,"Token is operator, but not '=', thus token is illegal >%s<",token.token_name);
			parse_error(msg);
			return;
		}
	    ...
	    if (token_class != LABEL) {
            sprintf(msg,"Token is not operator, and not label, thus it is illegal >%s<",token.token_name);
            parse_error(msg);
